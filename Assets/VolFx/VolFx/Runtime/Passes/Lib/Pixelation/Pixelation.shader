...
//#endif
            ...
            {
                return float2(round((uv.x - .5) * _Pixels.x) / _Pixels.x + .5, round((uv.y - .5) * _Pixels.y) / _Pixels.y + .5);
            }
            ...
            half4 frag(const frag_in i) : SV_Target
            {
                ...
                snap.x -= snap.x % _MainTex_TexelSize.x;
                snap.x += _MainTex_TexelSize.x * .5;
                
                snap.y -= snap.y % _MainTex_TexelSize.y;
                snap.y += _MainTex_TexelSize.y * .5;
#endif
                ...
                //return half4(lerp(sample.rgb, _Color.rgb, shape).rgb, sample.a);
            }
            ENDHLSL
        }
    }
